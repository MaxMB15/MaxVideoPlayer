...
export function LiveInfoDrawer({ channel, onClose }: LiveInfoDrawerProps) {
  ...
  const progress = currentProgramme
    ? (now - currentProgramme.startTime) / (currentProgramme.endTime - currentProgramme.startTime)
    : 0;
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 pt-2 pb-4 shrink-0">
          ...
          <div className="flex flex-col justify-center gap-0.5 flex-1 min-w-0">
            ...
            <p className="text-sm font-semibold leading-tight truncate">{channel.name}</p>
          </div>
          ...
          <div className="px-5 py-4 shrink-0">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[11px] text-muted-foreground whitespace-nowrap shrink-0">
                {formatTime(currentProgramme.startTime)} → {formatTime(currentProgramme.endTime)}
                &ensp;({formatDuration(currentProgramme.startTime, currentProgramme.endTime)})
              </span>
            </div>

            {/* Title + description */}
            <p className="text-sm font-semibold leading-tight mb-1">{currentProgramme.title}</p>
        ...
        <div className="flex flex-col flex-1 min-h-0">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center flex-1 py-8 px-5 gap-2 text-center">
              <p className="text-sm text-muted-foreground">Schedule unavailable for this channel.</p>
              <p className="text-xs text-muted-foreground/60">
                No EPG data matched. Configure an EPG URL in provider settings to enable programme guides.
              </p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div
                      className={`w-0.5 h-8 rounded-full shrink-0 ${
                        isCurrent ? "bg-primary" : isPast ? "bg-border" : "bg-border/50"
                      }`}
                    />
                    ...
                    <span
                      className={`text-xs tabular-nums shrink-0 w-12 ${
                        isPast && !isCurrent ? "text-muted-foreground/60" : "text-muted-foreground"
                      }`}
                    >
